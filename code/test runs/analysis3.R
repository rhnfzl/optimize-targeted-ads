#With all the non Third Party Atributes

# Library -----------------------------------------------------------------
library(tidyverse)
library(tidymodels)
library(neuralnet)
#set.seed(2020) # Setting random number seed so 

# Data Wrangling ----------------------------------------------------------

_dset <- read_csv("./data/_dset.csv")  %>% 
  select(-X1)
...
_dset <- _dset %>% mutate(dear_john = as.character(dear_john))
...
train_recipe <- training_dset %>% 
  recipe(cont_subs ~ series_hour + counties +
  ...
  step_dummy(watch_medium, gift, genre, counties, one_hot = TRUE) %>% # create one hot encoding for categorical variable
...
# Same steps to the test dataset  
...
mymodel <- neuralnet(
  ...
  formula = cont_subs == "Yes" ~ series_hour + movie_hour + age + cont_watch + counties_Kings + 
    counties_Los.Angeles + counties_Monterey + counties_Napa + counties_San.Diego + counties_San.Fransisco + 
    counties_Santa.Barbara + counties_Sonoma + counties_Trinity + counties_Ventura +
  ...
  hidden = 10 # specifying the length of hidden layers inside the neural network (at the moment an aritrary numbers we are not going to optimize the number)
)
